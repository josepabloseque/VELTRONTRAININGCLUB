...
export const TermsModal: React.FC<TermsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Card */}
      <div
        className="relative z-10 w-full max-w-lg max-h-[85vh] flex flex-col bg-[#121514] border border-[#8E8C3A]/40 rounded-2xl text-white shadow-[0_20px_60px_rgba(0,0,0,0.95)]"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        <div className="flex items-center justify-between p-5 sm:p-6 border-b border-zinc-800/80 shrink-0">
          <div>
            <h2 className="font-bebas text-xl sm:text-2xl tracking-wide uppercase text-white leading-none">
              Términos y Condiciones
            </h2>
            <p className="text-[11px] text-[#B5B04E] font-barlow uppercase tracking-wider mt-1">
              Veltron Training Club
            </p>
          </div>

          <button
            onClick={onClose}
            className="text-zinc-400 hover:text-white p-2 rounded-xl bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800/80 transition-colors flex items-center justify-center"
        ...
        {/* Scrollable Content */}
        <div className="overflow-y-auto p-5 sm:p-6 space-y-5 text-xs text-zinc-300 font-barlow leading-relaxed">
          <section className="space-y-1.5">
            <h3 className="text-sm font-bold text-white uppercase font-barlow tracking-wide">
        ...
        {/* Footer */}
        <div className="p-4 sm:p-5 border-t border-zinc-800/80 bg-[#0A0C0B] rounded-b-2xl shrink-0">
          <button
            onClick={onClose}
            className="w-full py-3 bg-[#8E8C3A] hover:bg-[#B5B04E] text-black font-bebas text-base tracking-wider uppercase rounded-xl transition-all shadow-md active:scale-95 leading-none"
          >
            Entendido y Aceptar
          </button>
        </div>
